...
const int startLedIndicator[] = {8, 9, 10};
...
/*	
	Condition to win: Led,Button: {5,11}, {6,12}, {7,13}
    Logic to win LedPin[i] = ButtonPin[i] + 6
*/
...
void finalResult() {
    ...
    for (int i = 0;i < numButtons; i++){
  	    if((nextButtonOrder[i] + 6) == orderLedBlink[i]){
     	    cont = cont+1;
        } 
    }
    ...
    if (passed) {
      // const int startLedIndicator[] = {9, 10, 11};  R/G/B
        // Green Light
        digitalWrite(startLedIndicator[0], LOW);
        digitalWrite(startLedIndicator[1], HIGH);
